...
export default Vue.extend({
  ...
  methods: {
    ...
    uploadImage(file: any) {
      ...
      this.$mainApi.toolsApi.upload(form).then((res) => {
    ...
    submit() {
      this.loading = true;
      if (this.formId) {
        this.$mainApi.activityApi.putActivityById(this.formId.toString(), this.formData).then((res) => {
        ...
        this.$mainApi.activityApi.postActivity(this.formData).then((res: any) => {
          this.$message.success("提交成功");
          this.$emit('success');
          this.visible = false;
        }).finally(() => {
