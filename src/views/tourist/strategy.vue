...
<script lang="ts">
import Vue from 'vue';
import StrategyFormDialop from "./components/strategyFormDialop.vue";
import DeleteDialop from "@/components/Dialops/deleteDialop.vue";
...
export default Vue.extend({
  ...
  methods: {
    ...
    getTableData() {
      this.loading = true;
      this.$mainApi.strategyApi.getStrategyByPage({
    ...
    tablePageSizeChange(val: number) {
      this.tableCurrentPageSize = val;
    },
    tablePageCurrentChange(val: number) {
    ...
    showStrategyFormDialop(id: number, formData: any) {
      (this.$refs.StrategyFormDialop as any).init(id, formData);
    },
    showDeleteDialop() {
      ...
      this.tableSelected.forEach((selected: any) => {
        deleteList.push({
          id: selected.id,
    ...
    deleteData(list: Array<any>) {
      const allPromise: Array<any> = [];

      list.forEach((item) => {
        allPromise.push(this.$mainApi.strategyApi.deleteStrategyById(item.id));
      });
